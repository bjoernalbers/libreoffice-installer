...
func TestChecksumURL(t *testing.T) {
	tests := []struct {
		...
		},
		{
			"7.4.6",
			"arm64",
			"https://download.documentfoundation.org/libreoffice/stable/7.4.6/mac/aarch64/LibreOffice_7.4.6_MacOS_aarch64.dmg.sha256",
			false,
		},
		{
			"7.5.2",
		...
		},
		{
			"7.5.2",
			"arm64",
			"https://download.documentfoundation.org/libreoffice/stable/7.5.2/mac/aarch64/LibreOffice_7.5.2_MacOS_aarch64.dmg.sha256",
			false,
		},
		{
			"7.4.6",
	...
	for _, test := range tests {
		url := URL{test.version, test.arch}
		got, err := url.Checksum()
		if (err != nil) != test.wantErr {
			t.Errorf("%#v.Checksum() err = %v, wantErr: %v", url, err, test.wantErr)
		}
		if got != test.want {
			t.Errorf("%#v.Checksum():\ngot:\t%q\nwant:\t%q", url, got, test.want)
		}
